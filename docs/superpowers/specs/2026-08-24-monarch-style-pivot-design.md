...
- **Token cascade:** because the app's colors are global CSS custom properties (`--ink`, `--paper`, `--amber`, `--teal`, ... in `assets/styles.css`'s `:root` block) referenced by every tab's rules, redefining them re-themes the *entire app*, not just the tabs this spec adds. This is treated as correct and intentional — a mixed old/new palette would look broken — not scope creep.
...
- Dashboard's existing chart *content* is not removed or restructured in this pass — it keeps its current six charts, just re-themed. A follow-up could trim charts that now duplicate Cash Flow/Spending, but that's out of scope here (avoids removing a just-shipped, reviewed feature as an unplanned side effect).
...
--mono: "Inter", ui-monospace, SFMono-Regular, Menlo, monospace; /* was "JetBrains Mono" — token NAME kept for tabular-numeral contexts, now points at Inter, not a separate monospace family */
--disp: "Sora", system-ui, -apple-system, Segoe UI, sans-serif; /* was "Space Grotesk" */
```

Tokens from the prior pilot's revamp (`--radius-sm`, `--radius-md`, `--card-bg`, `--shadow-card`, `--shadow-card-hover`, `--shadow-focus`, `--motion-fast/base/slow/ease`, `--cat-color-0..7`) are kept as token *names* (nothing that references them needs renaming) but several get new values:

```css
--radius-sm: 10px;      /* was 8px */
--radius-md: 18px;      /* was 12px — bigger, more "card" than "panel" */
--card-bg: #ffffff;     /* unchanged */
--shadow-card: 0 2px 8px rgba(20,21,26,.06), 0 12px 32px -8px rgba(20,21,26,.14); /* more visible than the pilot's subtle version */
--shadow-card-hover: 0 4px 12px rgba(20,21,26,.08), 0 20px 48px -12px rgba(20,21,26,.20);
--shadow-focus: 0 0 0 3px rgba(0,163,137,.35); /* now brand-teal, was amber */
```
...
const TEAL = "#0f766e", AMBER = "#b45309", RED = "#b3261e", BLUE = "#1d4ed8", PURPLE = "#7c3aed", SAND = "#c6bcab";
...
```js
const INK = "#14151a", INK3 = "#8a8d99", RULE = "#e7e8ed";
const TEAL = "#00a389", AMBER = "#ff6b4a", RED = "#e5484d", BLUE = "#3b82f6", PURPLE = "#a855f7";
const SAND = "#c7cad1"; /* was #c6bcab — kept as a name, mid-gray-blue instead of the old warm tan, still reads as "neutral" against the new palette */
const CATEGORY_SPECTRUM = ["#3b82f6","#a855f7","#ec4899","#f59e0b","#10b981","#06b6d4","#f97316","#8b5cf6","#ef4444","#14b8a6","#eab308","#84cc16"];
```
...
function cashFlowData(rows, month, year) {
  const inScope = rows.filter(
    (r) => Number(String(r.date).slice(0, 4)) === year &&
...
export function cashFlow(flows) {
  const flowColorFor = (label) =>
    label === "Income" || label === "Savings" ? TEAL
...
```js
const spendRows = [...a.catRows].filter((r) => r.actual > 0).sort((x, y) => y.actual - x.actual);
const top6 = spendRows.slice(0, 6);
const otherTotal = spendRows.slice(6).reduce((sum, r) => sum + r.actual, 0);
const donutRows = otherTotal > 0 ? [...top6, { category: "Other", actual: otherTotal }] : top6;
```
...
export function spendingDonut(rows, onSliceClick) {
  mount("c-spend-donut", {
    ...
    options: {
      responsive: true, maintainAspectRatio: false, animation: drawIn,
